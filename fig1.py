...
def parse_command_line():
    ...
    opts.var_1 = "tau_block"
    opts.var_2 = "tau_attestation"
    opts.x_scale = "log"
    opts.c_scale = "log"

    opts.output_path = "./figures/fig1"
...
if __name__ == "__main__":
    ...
    z3 = "threshold"
    def def_value():
        return {options.var_1: [], options.var_2: [], z1: [], z2:[], z3:[]}
    ...
    for i in df.index:
        ...
        er_diameter = np.log(tmp['no_nodes'])/np.log(tmp['no_nodes']*tmp['no_neighs']/(tmp['no_nodes']-1))
        d[(tmp[options.control_1], tmp[options.control_2])][z3].append(er_diameter*tmp[options.var_1])
