...
class Gasper:
    ...
    def lmd_ghost(self, attestations):
        ...
        for slot, node_attestations in fork_choice_attestations.items():
            heavyBlock = self.get_heaviest_block(
                slot, node_attestations, fork_choice_attestations, self.consensus_chain[slot-1])
            if heavyBlock == None:
                continue
    ...
    def get_latest_message_attest(self, block, attestations):
        ...
        block_parent_slot, _ = [(k, v) for k,
                                v in self.consensus_chain if v == block.parent][0]
        ...
        if block_parent_slot:
            ...
            if current_head.slot >= block.slot:
                return current_head
            ...
            if block.slot > current_head.slot:
                return current_head
    ...
    # def calculate_mainchain_rate(self, attestations):
    ...
    #     return len(main_chain)/len(self.consensus_chain.values())

    # def calculate_branch_ratio(self, attestations):
