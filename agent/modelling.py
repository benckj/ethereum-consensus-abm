...
class Model:
    ...
    def results(self, analyse_node=None, attack=None):
        ...
        """
        if attack:
            results_dict = [self.results(v) for v in np.random.default_rng().choice(self.validators,5)]

            return {
                "mainchain_rate": np.mean([x["mainchain_rate"] for x in results_dict]),
                "branch_ratio": np.mean([x["branch_ratio"] for x in results_dict]),
                "blocktree_entropy": np.mean([x["blocktree_entropy"] for x in results_dict]),
                "attack_probability": np.mean([x["attack_probability"] for x in results_dict]),
                "successful_reorgs": np.median([x["successful_reorgs"] for x in results_dict]),
                "failed_reorgs": np.median([x["failed_reorgs"] for x in results_dict]),
                "malicious_blocks_percent": np.mean([x["malicious_blocks_percent"] for x in results_dict]),
            }
        else: 
            if analyse_node:
            ...
            results_dict = {
                ...
                "attack_probability": malicious_slot_count / (self.chain_state.slot-1),
