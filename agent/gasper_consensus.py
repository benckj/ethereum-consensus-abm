...
class Gasper:
    ...
    def lmd_ghost(self, local_blockchain, attestations):
        ...
        self.consensus_chain = {slot: block for slot, block in self.consensus_chain.items() if slot <= self.finalized_head_slot}

        while(previous_head):
            known_children = [block for block in previous_head.children if block in local_blockchain]

            if len(known_children) == 0:
                return

            block_weights = {block: self.get_cummulative_weight_subTree(block, attestations) for block in known_children}

            block_with_max_votes = [block for block, weight in block_weights.items() if weight == max(block_weights.values())]
            ...
            if len(block_with_max_votes) > 1:
                block_with_max_votes = [block for block in block_with_max_votes if block.slot == min([block.slot for block in block_with_max_votes])]

            heavyBlock = min(block_with_max_votes, key=lambda block: block.value)
